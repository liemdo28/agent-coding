...
## 2. The two SKUs — Personal & Pro

### Personal — $49/month or $490/year

For individual engineers who want sovereignty.

- **Full Local Agent** — all features, all pillars
- **Single machine** — one laptop/workstation
- **Local Ollama models** — qwen2.5-coder, deepseek-r1, llama3
- **Knowledge Base** — up to 50K documents
- **Git Intelligence** — full autonomy: auto-commit, auto-push, auto-merge
- **Code Scanning** — AST parsing, security analysis, test generation
- **Telegram Bot** — command interface for /scan, /test, /fix, /push
- **Watcher Daemon** — background monitoring of ~/Projects/
- **Free KB updates** — signed artifact downloads
- **Community support**

**Target:** Individual engineers at regulated companies, security researchers, developers who care about owning their tools.

### Pro — $199/month or $1,990/year

For professional developers and small teams who need more.

- **Everything in Personal** — plus:
- **Multi-model routing** — Local Ollama + Claude API + OpenAI API + Antigravity IDE
- **Team Memory Sync** — LAN-based shared knowledge base
- **RBAC** — role-based access control (viewer/contributor/reviewer/admin)
- **Compliance reports** — SOC2, HIPAA, FedRAMP, IRAP, ISO 27001 documentation
- **Priority support** — 24/7 response
- **Fine-tune pipeline** — customize model on your own code (runs locally)
- **Multi-agent coordination** — Architect + Specialist agents working together
- **Unlimited KB documents** — no cap
- **Enterprise features** — air-gapped deployment, federated multi-site

**Target:** Small engineering teams (2-20), startups in regulated industries, professional developers who need more power.

---

## 3. The five-year vision: what Local Agent becomes
...
## 4. What the agent must be able to do (the senior-engineer-times-ten test)
...
### 4.1 The Coding Mastery Test — what V1 must clear
...
### 4.3 The Accounting Mastery Test
...
## 5. The four provider integration

### Local Ollama (always available)
- qwen2.5-coder:7b — primary coding model
- qwen2.5-coder:1.5b — intent parsing, fast tasks
- deepseek-r1:7b — reasoning and debugging
- llama3:latest — general purpose backup

### Claude (Anthropic) — Pro only
- claude-3-5-sonnet-20241022 — primary cloud model
- claude-3-5-opus-20241022 — complex reasoning
- Requires API key (never stored, uses env var)

### OpenAI Codex — Pro only
- gpt-4o — general purpose
- gpt-4-turbo — faster option
- Requires API key (never stored, uses env var)

### Antigravity (Google DeepMind IDE) — Pro only
- Multi-agent autonomous execution
- Gemini 3 Pro + Claude Sonnet 4.6/Opus 4.6 + GPT-OSS
- Fork of VS Code, launched Nov 2025
- Connects via MCP (Model Context Protocol) or IDE IPC

---

## 6. The seven pillars (was three — we expand)
...
The depth defined in §4.1 above. This is what we are most known for. This is where we go furthest beyond any competitor.

### Pillar 2 — Data Analysis
The depth defined in §4.2. Strictly advisory. Powered by DuckDB + Arrow + local stat libraries.

### Pillar 3 — Accounting & Financial Operations
The depth defined in §4.3. Strictly advisory. The hash-chained audit ledger we already have becomes the foundation for an industry-standard format.
...
The V1 list had 8. The V3 list has 15. Every PR is measured against these. Violation = revert.
...
## 8. Anti-features (expanded — these will not exist, ever)

- ❌ Cloud-hosted version (any kind)
- ❌ "Pro" tier with cloud LLM fallback (Personal = local only, Pro = local + cloud)
...
**Eat what we ship.** Our internal engineering runs on Local Agent. No cloud AI on customer code. No exceptions, including for emergencies. If the agent can't help with an emergency, the agent needs to be better, not bypassed.
