...
#   2.  Use the local repo, optionally sync branch, verify baseline CI is green
...
#
# Env:
#   REPO_DIR=/path/to/repo       default: this script's repository root
...
#   SKIP_MODEL_PULL=1            skip the model downloads (default: 1)
#   SKIP_VERIFY=1                skip the existing CI run
#   SKIP_INSTALL=1               skip npm ci
#   ALLOW_GIT_SYNC=1             allow git fetch/pull/checkout (default: 0)
# ============================================================================
...
ALLOW_GIT_SYNC="${ALLOW_GIT_SYNC:-0}"
STAGE_BOOTSTRAP_OUTPUTS="${STAGE_BOOTSTRAP_OUTPUTS:-0}"
...
sub "Compute"
if df -g "$PWD" >/dev/null 2>&1; then
  DISK_FREE_GB=$(df -g "$PWD" | awk 'NR==2 {print $4}' | tr -d 'G ')
else
...
if [ -d "$REPO_DIR/.git" ]; then
  ...
  if [ "$ALLOW_GIT_SYNC" = "1" ]; then
    (cd "$REPO_DIR" && git fetch --all --prune >/dev/null 2>&1) || warn "git fetch had warnings"
  else
    warn "git sync disabled (set ALLOW_GIT_SYNC=1 to fetch/pull)"
  fi
else
  if [ "$ALLOW_GIT_SYNC" = "1" ]; then
    ...
    fail "Repo not found at $REPO_DIR. Set REPO_DIR or ALLOW_GIT_SYNC=1 to clone."
  fi
fi
...
if [ "$SKIP_INSTALL" = "1" ]; then
  warn "Skipping dependency install (SKIP_INSTALL=1)"
else
  sub "Installing dependencies"
  npm ci --no-audit --no-fund --loglevel=error 2>&1 | tail -2
  ok "root deps installed"
  (cd accounting-engine && npm ci --no-audit --no-fund --loglevel=error 2>&1 | tail -2)
...
if [ "$SKIP_VERIFY" = "1" ]; then
  ...
  sub "Running baseline CI"
  npm run setup:production -- --skip-install >/dev/null && ok "production setup verification passed" || fail "production setup verification failed"
fi
...
if [ "$STAGE_BOOTSTRAP_OUTPUTS" = "1" ]; then
  git add "$BASELINE_FILE" docs/adr/ scripts/m-verify/ 2>/dev/null || true
  ok "bootstrap outputs staged"
else
  warn "bootstrap outputs left unstaged (set STAGE_BOOTSTRAP_OUTPUTS=1 to stage them)"
fi
...
cat <<EOF
  ...
  Required reading (today, before any code):
    1. docs/vision/MANIFESTO_v2.md  — read twice, sleep on it, read once more
    2. docs/vision/DEV_BUILD_GUIDE_v2.md — read end-to-end at least once
