...
import { loadFilePurposeIndex, buildFilePurposeIndex } from './FilePurposeIndexer.js';
import { getImplementationMap, writeImplementationMap } from './ArchitectureSummarizer.js';
import { listKnownIssues, listBlockers, listRisks } from './EngineeringStateTracker.js';
import { generateContextPriorityMd } from './ContextPriorityManager.js';
...
export function generateLatest(workspaceRoot, opts = {}) {
  ...
  const knownIssues = listKnownIssues(workspaceRoot);
  const blockers    = listBlockers(workspaceRoot);
  const risks       = listRisks(workspaceRoot);
  const implMap     = getImplementationMap();

  // Build/refresh file purpose index
  const filePurposeIndex = buildFilePurposeIndex(workspaceRoot);
  const topFiles = Object.entries(filePurposeIndex)
...
${risks.filter((r) => r.status !== 'mitigated').length
  ? risks.filter((r) => r.status !== 'mitigated').map((r) => `- **[${r.severity.toUpperCase()}]** ${r.title}`).join('\n')
  : '- None critical — all known risks mitigated (see architecture/implementation-map.md)'}

## Known Issues
${knownIssues.length
  ? knownIssues.map((i) => `- **${i.id}** [${i.risk}] ${i.title}\n  Workaround: ${i.workaround}`).join('\n')
  : '- None currently'}

## Blocked Systems
${blockers.length
  ? blockers.map((b) => `- **${b.id}** [${b.severity}] ${b.system}: ${b.reason}`).join('\n')
  : '- None currently'}

## Implementation Map (Summary)
${implMap.slice(0, 10).map((m) => `- **${m.name}** (Phase ${m.phase}) — ${m.status} — ${m.purpose.slice(0, 80)}...`).join('\n')}
_Full map: \`local-agent logs architecture --file implementation-map\`_

## File Purpose Index (Key Files)
${topFiles.slice(0, 15).map(([file, info]) => `- \`${file}\`\n  → ${info.purpose}`).join('\n')}
_Search: \`local-agent logs file-purpose --search "<keyword>"\`_
...
1. \`local-agent logs file-purpose --search "<keyword>"\` — find relevant files without reading source
2. \`local-agent logs search "<query>"\` — search across all engineering log documents
3. \`local-agent os\` — check Engineering OS dashboard
4. \`local-agent qa\` — run QA if not done recently
5. \`local-agent logs checkpoint\` — record current progress

---
_Auto-generated by Engineering Build Log System. Do NOT edit manually._
_Policy: READ LOG FIRST. Read source files SECOND — only when log context is insufficient._
_Run \`local-agent logs update\` to refresh._
`;
...
// Also update system-architecture entry point count reference
...
export function writeArchitectureDocs(workspaceRoot) {
  ...
  // Phase 55 — implementation map + context priority
  writeImplementationMap(workspaceRoot);
  writeFileSync(join(dir, 'context-priority.md'), generateContextPriorityMd(workspaceRoot));
}
